<?php
	...
	$sql = $databasePDO->query("select distinct r.count as conteo, c.name as comuna from reporte r, comuna c where st_intersects(c.geom,st_setsrid(st_makepoint (r.x,r.y),4326)) group by c.name");
...
<body>
	...
	<h1>Cantidad De Noticias Por Comuna</h1>
		<br>
		<div>
				...
				<tbody>
					<?php foreach($resultados as $edificio){ ?>
						<tr>
							...
							<td><?php echo $edificio->comuna ?></td>
